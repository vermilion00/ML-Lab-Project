...
#Extra frame for the file path labels
file_frame = ttk.Frame(root, padding="2 0 2 0")
file_frame.bind('<Enter>', lambda a, m="file_path_label": hint_text.set(HINT_TEXT[m]))
file_frame.pack(anchor=W, padx=2)
load_file_label = ttk.Label(file_frame, text="Selected file path(s): ").pack(side=LEFT, anchor=NW)
load_file_path_label = ttk.Label(file_frame, textvariable=file_path, wraplength=500).pack(side=RIGHT)
