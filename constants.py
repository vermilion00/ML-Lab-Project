...
HINT_TEXT = {
    ...
    "slow_features_box": "Choose whether you want to use slow features like harmony, perceptr and tempo. Using them will make the extraction progress take ~3-8x as long. Models can only predict songs that were extracted with the same setting.",
...
MODEL_LOADING_FAILED_MSG = "An error has occured when loading the model. Make sure you have selected the correct file, and that the model you're trying to load was generated using this program."
...
OPTIONS_MISSMATCH_1_MSG = "The model was trained using the slow features, but your currently selected features were extracted without them. Continuing with the prediction will likely yield a worse result. Would you still like to continue?"
OPTIONS_MISSMATCH_2_MSG = "The model was trained without using the slow features, but your currently selected features were extracted with them enabled. Continuing with the prediction will likely yield a worse result. Would you still like to continue? If you wish to change this setting, you can find it in the options menu."
OPTION_MISMATCH_1_MSG = "The model was trained including the slow features, but you are about to extract files without using them. Predicting the genre with this mismatch is not possible. Would you like to change the setting to match the model and continue extracting?\nClick Yes to change the setting,\nClick No to extract without changing the setting,\nClick Cancel to abort the extraction."
OPTION_MISMATCH_2_MSG = "The model was trained without the slow features, but you are about to extract files while using them. Predicting the genre with this mismatch is not possible. Would you like to change the setting to match the model and continue extracting?\nClick Yes to change the setting,\nClick No to extract without changing the setting,\nClick Cancel to abort the extraction."
